...
pub fn init(allocator: std.mem.Allocator) XChaCha20 {
    return XChaCha20{
        .allocator = allocator,
    };
}
...
pub fn encrypt(self: *XChaCha20, reader: *std.Io.Reader, writer: *std.Io.Writer, key: []const u8) !void {
    const ctx = Context{ .key = key };
    var core = Core.init(self.allocator);
    try core.encrypt(reader, writer, ctx, computeNonce, encryptChunk);
}

pub fn decrypt(self: *XChaCha20, reader: *std.Io.Reader, writer: *std.Io.Writer, key: []const u8) !void {
    const ctx = Context{ .key = key };
    var core = Core.init(self.allocator);
